...
function _drawRocket(frame, cx, cy) {
  ...
  for (let dy = 0; dy < 9; dy++) {
    const hw = Math.max(0, 3 - Math.round(dy * 3 / 9));
    const col = dy < 4 ? 7 : 8;
    for (let dx = -hw; dx <= hw; dx++) _px(frame, cx + dx, cy + 7 + dy, col);
  }
}
...
function _drawTrail(frame, cx, cy) {
  ...
  for (let dy = 0; dy < 55; dy++) {
    const ty = trailTop + dy;
    if (ty < 0 || ty >= _GIF_H) continue;
    const fade = 1 - dy / 55;
    const c = fade > 0.55 ? 1 : fade > 0.2 ? 2 : 3;
    _px(frame, cx,     ty, c);
    _px(frame, cx - 1, ty, c);
    if (dy < 30) _px(frame, cx + 1, ty, c);
  }
}
...
function buildHireBlocks({ candidateName, role, location, recruiter }) {
  return [
    {
      type: 'image',
      image_url: 'https://hire-bot-032u.onrender.com/hire-gif',
