...
function buildHireBlocks({ candidateName, role, location, recruiter }) {
  return [
    ...
    {
      type: 'section',
      text: { type: 'mrkdwn', text: `Welcome *${candidateName}* — we're glad you're here.` }
    },
    {
      type: 'section',
      fields: [
...
app.post('/slash-hired', async (req, res) => {
  ...
  if (parts.length < 3 || parts.some((p) => !p)) {
    return res.json({
      ...
      text: ':warning: Use: `/hired [Name] | [Role] | [Location]`'
    });
  }
  const [candidateName, role, location] = parts;
  const recruiter = user_name || `<@${user_id}>`;
  res.json({ response_type: 'ephemeral', text: `:rocket: Posting announcement for *${candidateName}*...` });
  try {
    await slack.chat.postMessage({
      channel: CHANNEL,
      text: `:rocket: New hire: ${candidateName} — ${role} at Nominal`,
